...
- **CSV files**  
  ...
  > **Note on FID/KID confidence intervals:** The files in `results/dreambooth/` and
  > `results/textual_inversion/` were produced by the original H200 run and use a bootstrap
  > that resampled both the generated *and* the real images, which inflated FID/KID and caused
  > some intervals to not contain the point estimate (e.g., Paçoca FID 227.8 with CI [232.7,
  > 260.9]). The numbers reported in the paper (Tables 2–3 and Figure 3) come from
  > `results/recomputed/metrics_recomputed.csv`, generated by `scripts/recompute/` on a CUDA
  > machine. That recomputation keeps the 20 real images fixed during bootstrap (resampling only
  > the generated images), so all intervals are centered on the point estimate. The
  > `experiment_utils.py` bootstrap has been corrected accordingly.
