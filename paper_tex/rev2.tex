...
\textbf{\textcolor{blue}{Response:}} We thank the reviewer for this important suggestion. We would like to clarify that our training pipeline is in fact based on the DreamBooth paradigm, combined with LoRA for parameter-efficient adaptation in the few-shot setting. We agree that this was not sufficiently explicit in the previous version, and we have now clarified this point in the revised manuscript. In addition, following the reviewer’s recommendation, we included an alternative baseline comparison using \texttt{Textual Inversion} with the SDXL Model, under the same concept/data conditions adopted in our experiments (Section 3.4). We report quantitative results (CLIP Score, FID, and KID, over 1000 generated images per condition) in Table 3 and qualitative visual examples in Figure 4, in the new ``Comparison with Textual Inversion'' subsection (Section 4.3), allowing a direct comparison between our DreamBooth-based pipeline and the Textual Inversion baseline. DreamBooth+LoRA obtains a better CLIP Score and FID for all eight concepts and a better KID for six of the eight. This addition strengthens the benchmarking analysis and better positions the contribution of our approach.
...
First, we increased the number of generated samples used for quantitative evaluation from 100 to 1000 images per concept (Table 2), which provides a substantially more stable basis for FID/KID estimation; the checkpoint-wise curves in Figure 3 keep 100 images at the five intermediate checkpoints, since only the trajectory's endpoints are used for the conclusions in Table 2. Second, in addition to CLIP Score and FID, we now include Kernel Inception Distance (KID) (Section 3.7.3, Table 2), which is generally considered more robust in limited-sample regimes and therefore serves as a useful complementary metric. Third, we report uncertainty for all quantitative metrics by providing mean, standard deviation, and 95\% confidence intervals obtained via bootstrap resampling with 1000 resamples (Table 2, Section 3.7.3). This allows the comparison between methods and concepts to be interpreted in a statistically more grounded way rather than relying on single-point estimates only. Fourth, we added a group-wise breakdown of these metrics across the three concept categories used in this study (foods, folklore beings, artifacts/clothing; Section 4.2), reporting the range across the two or three concepts in each group rather than a bootstrap interval, since a formal confidence interval is not meaningful at that sample size.
...
\textbf{\textcolor{blue}{Response:}} We thank the reviewer for this important comment. In response, we added a short \emph{Hardware} paragraph in the \emph{Model Training} subsection of the revised manuscript, reporting the main computational resources used to run our experiments. The results in this revision were produced on a single NVIDIA H200 GPU (143\,GB VRAM), 256 logical CPUs, and approximately 1.5\,TB of system RAM; peak GPU memory usage was approximately 25\,GB for both DreamBooth+LoRA and Textual Inversion training, and each 3000-step training run took approximately 27 minutes (DreamBooth+LoRA) or 20 minutes (Textual Inversion) when run alone on the GPU. We believe this addition improves the reproducibility of the proposed pipeline and facilitates a more accurate assessment of the computational requirements for training and inference.
...
\textbf{\textcolor{blue}{Response:}} We thank the reviewer for this valuable comment. We agree that, in the previous version, the rationale behind the choice of concepts and the scope of generalization were not sufficiently explicit. In the revised manuscript, we clarify that the selected concepts were not intended as an arbitrary list, but rather as culturally underrepresented examples chosen to span distinct visual categories with different representation challenges.

More specifically, we now organize the benchmark concepts into three semantic groups: \emph{foods} (e.g., \textit{cuscuz}, \textit{lokum}, \textit{pa\c{c}oca}), \emph{folklore/mythological beings} (e.g., \textit{saci}, \textit{chaneques}), and \emph{artifacts/clothing/objects} (e.g., \textit{patu\'a}, \textit{jian}, \textit{chamanto}). This grouping allows us to evaluate whether the proposed pipeline behaves consistently across concept types with different visual properties, such as texture-heavy food items, character-like entities with higher semantic ambiguity, and structured objects or garments with more stable geometric cues.

Following the reviewer’s suggestion, we added a group-wise analysis in addition to the per-concept results. In particular, we now report grouped quantitative results for CLIP Score, FID, and KID, together with their uncertainty estimates, so that the reader can assess performance trends not only at the individual concept level but also at the category level. This provides a clearer picture of how the method behaves across different types of cultural concepts.

At the same time, we slightly refined the wording of our claims in the manuscript. Rather than suggesting unrestricted generalization to all cultural concepts, we now state the conclusion more precisely: the proposed pipeline shows robust behavior across multiple categories of underrepresented cultural concepts represented in our benchmark. We believe this revised framing, together with the new group-wise analysis, addresses the reviewer’s concern and better situates the scope of the contribution.
\end{enumerate}
...
\textbf{\textcolor{blue}{Response:}} We thank the reviewer for this comment. In response, we expanded the Methodology section to clarify key dataset details and the data-handling steps used in our experiments. In particular, we now make explicit the dataset size used for fine-tuning (a fixed set of approximately 20 images per concept in our study, 19 for Saci), describe the practical cleaning/quality-control mechanisms (user-facing visual inspection and filtering of retrieved samples, with optional manual upload of curated images), and clarify that the pipeline’s data sourcing is not inherently tied to a single web platform and can be adapted to copyright-safe repositories depending on the deployment context.

Regarding evaluation, we agree that CLIP Score and FID are standard metrics and do not fully capture cultural fidelity. We therefore position them as quantitative proxies that complement our qualitative visual assessment (Figures 1 and 2). As detailed in our response above on evaluation metrics and sample size, we have since added KID and 95\% bootstrap confidence intervals (Table 2, Section 3.7.3), so the quantitative comparisons in this revision are no longer single-point estimates. What remains future work, and is acknowledged as such in the revised Conclusions, is human evaluation and validation by cultural experts or affected communities, which automatic metrics and confidence intervals cannot substitute for.
...
\textbf{\textcolor{blue}{Response:}} We thank the reviewer for this valuable suggestion. We agree that comparing purely web-collected data against explicitly curated cultural repositories would be an interesting and worthwhile extension. However, we would like to clarify that, in the current pipeline, the images used for fine-tuning are not consumed in a fully automatic or unfiltered manner. After the web-scraping stage, the retrieved images are presented to the user in a visual inspection interface, where a human verification step is performed before training. In this step, irrelevant, low-quality, duplicated, or culturally misleading images can be removed manually. Therefore, the datasets used in our experiments are already \emph{human-curated} subsets of the scraped results, rather than raw ``web-only'' collections.

We have clarified this point more explicitly in the revised manuscript, since we agree that it is central to understanding the practical quality-control mechanism of the proposed pipeline. In addition, we now stress that the framework is not tied to a single source such as a general-purpose search engine: the same workflow can ingest images from curated archives, museum collections, open-licensed repositories, or manually provided datasets. In this sense, the contribution of the paper is the automation and human-guided curation pipeline itself, which is compatible with both scraped and pre-curated sources.

Given the revision timeframe, we did not add a separate ``raw web-only'' versus ``external curated repository'' benchmark in this version. Nevertheless, we now make clear that all training sets used in the reported experiments underwent human verification and manual filtering, which already introduces a curation stage prior to fine-tuning. We also highlight multi-source and source-comparison experiments as an important direction for future work.
...
\textbf{\textcolor{blue}{Response:}} We thank the reviewer for this constructive suggestion. We agree that, beyond presenting an automated pipeline, the paper is strengthened when it extracts reusable practical knowledge from the experimental results. In the revised version, we move in this direction by expanding the analysis from individual concepts to \emph{group-wise} behavior across different semantic categories (foods, folklore/mythological beings, and artifacts/clothing/objects), and by reporting performance trends across checkpoints, methods, and concept groups.

More specifically, the revised evaluation pipeline now supports per-concept and per-group analyses using CLIP Score, FID, and KID, with uncertainty estimates and checkpoint-wise comparisons. This allows us not only to compare methods, but also to observe how training dynamics differ across concept types. Based on these analyses, we reframed the discussion to provide practical guidance on how the pipeline behaves across categories, which is a first step toward data-driven rule extraction for underrepresented concept adaptation.

While we do not claim a fully automatic rule-inference module in the present paper, we now position the work more clearly as an automated pipeline accompanied by empirically grounded configuration insights. In other words, the revised manuscript goes beyond a purely operational system description and includes a structured analysis from which practical heuristics can be derived, such as the relative stability of different concept categories and the usefulness of checkpoint-wise evaluation when selecting training duration. We believe this addresses the reviewer’s concern within the scope of the current paper while also opening a clear path for future work on explicit automatic rule inference.
\end{enumerate}
