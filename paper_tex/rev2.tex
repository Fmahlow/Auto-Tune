...
Finally, we agree that automated metrics are only proxies for cultural fidelity. For this reason, we expanded the evaluation protocol to include richer qualitative comparisons and a human-evaluation stage focused on cultural recognizability and fidelity, with rating forms prepared for annotators familiar with the corresponding concepts. In the revised manuscript, we explicitly position CLIP, FID, and KID as quantitative proxies that complement---rather than replace---qualitative and human-centered assessment. We believe these changes substantially strengthen the rigor of the evaluation section and directly address the reviewer’s concern regarding metric adequacy and sample size.
